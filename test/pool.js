...
const JamesPool = artifacts.require('JamesPool')
...
contract('Pool', ([deployer, summoner, firstPoolMember, depositor, firstMemberKeeper, depositorKeeper, ...otherAccounts]) => {
  ...
  beforeEach('Deploy contracts', async () => {
    ...
    pool = await JamesPool.new(james.address, { gas: 8000000 })
  })
  ...
  async function giveAllowanceToJames (approver) {
    return token.approve(james.address, MAX_UINT256, { from: approver })
  }

  async function giveAllowanceToJamesPool (approver) {
  ...
  async function submitProposal (applicant, proposer, tribute, bonds, description) {
    ...
    if (tribute) {
      ...
      await giveAllowanceToJames(applicant)
    }
  ...
  describe('constructor', () => {
    it('should be initialized with the right values', async () => {
      ...
      assert.equal(await pool.james(), james.address)
  ...
  describe('When not active', () => {
    describe('activate', () => {
      ...
      it('Should fail if no allowance is given', async () => {
        await sendTokensTo(firstPoolMember, 1)
        // This should be rejected rejectedWith("JamesPool: Initial tokens transfer failed")
        // But the token used for testing doesn't return a boolean, but reverts on errors,
        // so the JamesPool contract never gets to return a revert reason.
        await pool.activate(1, 1, { from: firstPoolMember }).should.be.rejected
      })

      it('Should emit the SharesMinted event', async () => {
        await sendTokensTo(firstPoolMember, 1)
        await giveAllowanceToJamesPool(firstPoolMember)
    ...
    describe('deposit', () => {
      it("Shouldn't be callable", async () => {
        ...
        await giveAllowanceToJamesPool(deployer)
        await pool.deposit(123, { from: deployer })
          .should.be.rejectedWith('JamesPool: Not active')
      })
    })
  ...
  describe('When active', () => {
    ...
    describe('deposit', () => {
      ...
      it('Should be callable by anyone', async () => {
        for (const acc of otherAccounts) {
          ...
          await giveAllowanceToJamesPool(acc)
      ...
      it('Should fail if no allowance is given', async () => {
        ...
        // This should be rejected rejectedWith("JamesPool: Deposit transfer failed")
        // But the token used for testing doesn't return a boolean, but reverts on errors,
        // so the JamesPool contract never gets to return a revert reason.
    ...
    describe('keeperWithdraw', () => {
      ...
      it('Should transfer a proportional amount of tokens according to the bonds burnt, and burn the bonds, whithout affecting the keeper', async () => {
        ...
        await giveAllowanceToJamesPool(depositorKeeper)
    ...
    describe('James syncing', function () {
      this.timeout(10000)
      ...
      it('Should fail if called with a toIndex larger than the number of proposals', async () => {
        ...
        // https://github.com/nomiclabs/buidler/issues/272
        // TODO(@alcuadrado): Remove this when the issue gets fixed
        await pool.sync(1, { gas: 2000000 }).should.be.rejectedWith('JamesPool: Proposal index too high')

        await submitProposal(proposed, summoner, 0, 1, '')

        await pool.sync(1).should.be.fulfilled

        await pool.sync(2, { gas: 2000000 }).should.be.rejectedWith('JamesPool: Proposal index too high')

        await submitProposal(otherAccounts[0], summoner, 0, 1, '')

        await pool.sync(2).should.be.fulfilled

        await pool.sync(3, { gas: 2000000 }).should.be.rejectedWith('JamesPool: Proposal index too high')
      })
      ...
      it('Should emit the Sync event', async () => {
        ...
        await james.submitVote(0, VOTE_NO, { from: summoner })
      ...
      describe('When syncing a single proposal', () => {
        describe("When the proposal hasn't pass or hasn't been processed", () => {
          ...
          it("Shouldn't mint bonds if the proposal has been aborted, but it should increase the index", async () => {
            ...
            await james.abort(0, { from: proposed })
        ...
        describe('When the proposal is approved and processed', () => {
          describe("When the proposal isn't a grant", () => {
            it("Shouldn't mint bonds if the proposal isn't a grant (tokenTribute > 0), but it should increase the index", async () => {
              ...
              await advanceTimeInPeriods(ABORT_WINDOW_IN_PERIODS)
              await james.submitVote(0, VOTE_YES, { from: summoner })
              await advanceTimeInPeriods(VOTING_DURATON_IN_PERIODS + GRACE_DURATON_IN_PERIODS)
              await processProposal(0)
          ...
          describe('When the proposal is a grant', () => {
            ...
            describe("When somebody ragequits and nobody joins before it's processed", () => {
              it('Should mint bonds proportional to the max amount of the James on yest votes', async () => {
                ...
                await advanceTimeInPeriods(ABORT_WINDOW_IN_PERIODS)
                await james.submitVote(1, VOTE_YES, { from: summoner })
                await james.ragequit(1, { from: ignoredMember })
                await advanceTimeInPeriods(VOTING_DURATON_IN_PERIODS + GRACE_DURATON_IN_PERIODS)
      ...
      describe('When syncing multiple proposals', () => {
        it('Should sync up to the first non-processed proposal', async function () {
          ...
          await advanceTimeInPeriods(ABORT_WINDOW_IN_PERIODS)
          await james.submitVote(1, VOTE_YES, { from: summoner })
          await james.submitVote(2, VOTE_NO, { from: summoner })
          await advanceTimeInPeriods(VOTING_DURATON_IN_PERIODS + GRACE_DURATON_IN_PERIODS)
          await processProposal(1)
          ...
          await advanceTimeInPeriods(ABORT_WINDOW_IN_PERIODS)
          await james.submitVote(3, VOTE_YES, { from: summoner })
          await advanceTimeInPeriods(VOTING_DURATON_IN_PERIODS + GRACE_DURATON_IN_PERIODS)
          await processProposal(3)

          // Proposal 4
          await submitProposal(proposed4, summoner, 0, 2, '')
          await james.abort(4, { from: proposed4 })

          // Proposal 5
          await submitProposal(proposed5, summoner, 0, 2, '')
          await advanceTimeInPeriods(ABORT_WINDOW_IN_PERIODS)
          await james.submitVote(5, VOTE_YES, { from: summoner })
          ...
          await advanceTimeInPeriods(ABORT_WINDOW_IN_PERIODS)
          await james.submitVote(6, VOTE_YES, { from: summoner })
          await advanceTimeInPeriods(VOTING_DURATON_IN_PERIODS + GRACE_DURATON_IN_PERIODS)
          ...
          await assertBNEquals(james.totalShares(), 8)
  ...
  describe('Syncing proposals with huge descriptions', () => {
    it('should not OOG if the proposal being synced has a huge description', async function () {
      ...
      // This test is here to ensure that the JamesPool can't be locked by
